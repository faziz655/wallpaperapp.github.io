	...
	class API extends REST {
		...
		*/
		...
		public function get_wallpapers() {
			...
			$query = "SELECT g.id AS 'image_id', g.image_name, g.image AS 'image_upload', g.image_thumb, g.image_url, g.type, g.image_resolution AS 'resolution', g.image_size AS 'size', g.image_extension AS 'mime', g.view_count AS 'views', g.download_count AS 'downloads', g.featured, g.tags, c.cid AS 'category_id', c.category_name, g.last_update FROM tbl_category c, tbl_gallery g WHERE c.cid = g.cat_id AND c.category_status = '1' AND g.image_status = '1' AND $filter $order LIMIT $limit OFFSET $offset";
		...
		public function get_search() {
			...
			$offset = ($page * $limit) - $limit;
			$count_total = $this->get_count_result("SELECT COUNT(DISTINCT g.id) FROM tbl_gallery g, tbl_category c WHERE c.cid = g.cat_id AND c.category_status = '1' AND g.image_status = '1' AND (g.image_name LIKE '%$search%' OR g.tags LIKE '%$search%')");

			$query = "SELECT g.id AS 'image_id', g.image_name, g.image AS 'image_upload', g.image_thumb, g.image_url, g.type, g.image_resolution AS 'resolution', g.image_size AS 'size', g.image_extension AS 'mime', g.view_count AS 'views', g.download_count AS 'downloads', g.featured, g.tags, c.cid AS 'category_id', c.category_name, g.last_update FROM tbl_category c, tbl_gallery g WHERE c.cid = g.cat_id AND c.category_status = '1' AND g.image_status = '1' AND (g.image_name LIKE '%$search%' OR g.tags LIKE '%$search%') $order LIMIT $limit OFFSET $offset";
		...
		public function get_search_category() {
			...
			$query = "SELECT DISTINCT c.cid AS 'category_id', c.category_name, c.category_image, COUNT(DISTINCT g.id) as total_wallpaper
			FROM tbl_category c LEFT JOIN tbl_gallery g ON c.cid = g.cat_id WHERE c.category_status = '1' AND c.category_name LIKE '%$search%' GROUP BY c.cid ORDER BY c.cid DESC";
		...
		public function get_settings() {
			...
			if ($app_count > 0) {
				$respon = array('status' => 'ok', 'app' => $app, 'menus' => $menus, 'settings' => $settings, 'ads' => $ads, 'ads_status' => $ads_status);
			} else {
				$respon = array('status' => 'ok', 'app' => $app_array, 'menus' => $menus, 'settings' => $settings, 'ads' => $ads, 'ads_status' => $ads_status);
			}
